        ...
        <form method="POST" action="{{ route('posts.update', $post->id) }}">
            ...
            <div>
                <div class="mb-4">
                    ...
                    @error('title')
                        <p class="text-red-500 font-bold mt-3">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <textarea id="sub_title" name="sub_title"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('sub_title') border-red-500 @enderror"
                        rows="2" placeholder="Insert sub title">{{ old('sub_title') ?? $post->sub_title }}</textarea>
                    @error('sub_title')
                        <p class="text-red-500 font-bold mt-3">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <textarea id="body" name="body"
                        ...
                        rows="5" placeholder="Insert body">{{ old('body') ?? $post->body }}</textarea>
                    @error('body')
                        <p class="text-red-500 font-bold mt-3">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mb-4">
                    ...
                    <input id="published_at" name="published_at" value="{{ old('published_at') ?? $post->published_at }}"
